...
4. 點擊「在頁面開啟面板」可顯示側邊標註面板，並可在 popup 切換左右位置；面板中的下拉選單支援查看其他頁面，並能直接開啟對應頁面或捲動到本頁標註。於面板「頁面標籤」欄位可輸入或自動完成既有標籤，新增/移除特定來源的標籤，亦可使用關鍵字搜尋快速縮小結果。
5. 點擊已標註的文字會跳出設定面板，可調整顏色、刪除標註或編輯註解。
6. 重新整理或重新打開同一頁面時，標註與註解會自動回復。
